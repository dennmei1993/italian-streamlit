...
def _normalize_label(label: str) -> str:
    # Remove leading emojis/symbols and extra whitespace
    return re.sub(r"^[^A-Za-z]+\s*", "", (label or "").strip())

def _abs_repo_path(rel_path: str) -> str:
    if not rel_path:
        return ""
    base = os.path.dirname(__file__)
    return rel_path if os.path.isabs(rel_path) else os.path.join(base, rel_path)

def get_stage_assets_from_label(scenario_label: str) -> tuple[str, str]:
    """Return (background_path, avatar_path) for a scenario label.

    Works whether the scenario label includes emojis (from SCENARIOS) or not.
    """
    key = _normalize_label(scenario_label)
    bg = (STAGE_BACKGROUNDS.get(key) or [""])[0]
    av = (STAGE_AVATARS.get(key) or [""])[0]
    bg_abs = _abs_repo_path(bg)
    av_abs = _abs_repo_path(av)
    return bg_abs, av_abs
# ================== SESSION STATE ==================
...
# --- Scenario visuals (background + avatar) ---
# Uses repo assets/... paths. If missing, shows a clear warning.
_bg_path, _av_path = get_stage_assets_from_label(scenario_label)
if _bg_path and os.path.exists(_bg_path):
    st.image(_bg_path, use_container_width=True)
else:
    st.warning(f"Scenario background not found: {_bg_path or 'N/A'}")

# Avatar (optional)
if _av_path and os.path.exists(_av_path):
    # Keep avatar compact under the background
    st.image(_av_path, width=140)
else:
    st.info(f"Scenario avatar not found: {_av_path or 'N/A'}")
